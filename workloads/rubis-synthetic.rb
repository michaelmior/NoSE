...
$workload = NoSE::Workload.new do
  ...
  Q 'SELECT from_user_id, date, comment FROM comments.item_id WHERE item_id.id = ? ORDER BY comments.date'
  # 1. SELECT item_id as E_item, date as O_date, from_user_id, date, comment FROM comments;
  # I2227598752

  Q 'SELECT id, nickname, rating FROM users.region WHERE region.id = ? ORDER BY users.rating LIMIT 10'
  # 2. SELECT region as E_region, rating as O_rating, id, nickname, rating FROM users;
  # I1083340549

  Q 'SELECT id, name, description, max_bid FROM items.seller.region WHERE region.id = ?'
  ...
  Q 'SELECT from_user_id, date, comment FROM comments.item_id.seller.region WHERE item_id.quantity = ? AND region.id = ?'
  ...
  Q 'SELECT bid, date FROM bids.item_id.seller.region WHERE region.id = ? AND item_id.quantity = ? AND item_id.end_date < ?'
  ...
  Q 'SELECT from_user_id, comment, date FROM comments.item_id.seller WHERE seller.id = ?'
  ...
  Q 'SELECT id, name FROM items.category WHERE category.id = ? LIMIT 1000'
  # 7. SELECT category as E_category, id, name FROM items;
  # I3358488952

  Q 'SELECT comment FROM comments.item_id.category WHERE category.id = ? ORDER BY comments.date'
